function CKANServer() {
  ...
  this.basic_auth_user = "cioosatlantic";
  this.basic_auth_password = "4oceans";
  ...
  this.getURLParameterForBoundingBox = function(custom) {
    if (!custom) {
      ...
      ret +=
        this.bbox[0].toString() +
        "," +
        this.bbox[1].toString() +
        "," +
        this.bbox[2].toString() +
        "," +
        this.bbox[3].toString();
      return ret;
    } else {
      let ret = "ext_bbox="; //-104,17,-18,63
      ret +=
        this.spatialSearch.bbox[0].toString() +
        "," +
        this.spatialSearch.bbox[1].toString() +
        "," +
        this.spatialSearch.bbox[2].toString() +
        "," +
        this.spatialSearch.bbox[3].toString();
      return ret;
    }
  };
  ...
  this.getURLPaginated = function(startrow, numrow) {
    ...
    while (v < this.varriables.length) {
      ...
      if (varItem.checked) {
        if (this.support_eov) {
          filtered_query_elems = filtered_query_elems.concat(
            this.getVariableEOVFilters(varData)
          );
        } else {
          query_elems = query_elems.concat(
            this.getVariableEOVForQuery(varData)
          );
        }
      }
      ++v;
    ...
    if (this.support_time) {
      filtered_query_elems = filtered_query_elems.concat(
        this.getURLParamForTimeFilter()
      );
    }
    if (this.support_vertical) {
      filtered_query_elems = filtered_query_elems.concat(
        this.getURLParamForVerticalFilter()
      );
    }
...
function AddDisplayCKANExtent(data) {
  ...
  while (i < results.length) {
    ...
    if (objspatial != undefined) {
      ...
      feature.set(
        "center",
        getCenterOfCoordinates(objspatial["coordinates"][0])
      );
...
function lerpColor(color1, color2, lerpvalue) {
  ...
  let ret = [
    Math.round(color1[0] * col1mult + color2[0] * lerpvalue),
...
function getStyleFromClusterConfig(config, nbrElem) {
  ...
  if (nbrElem <= config["minimum"]["cluster_value"]) {
    ...
    let weigth =
      (nbrElem - config["minimum"]["cluster_value"]) /
      (config["maximum"]["cluster_value"] - config["minimum"]["cluster_value"]);
    maxweight = weigth;
    minweight = 1 - maxweight;
  }
  config["minimum"]["circle_radius"] * minweight +
    config["minimum"]["circle_radius"] * maxweight;

  ret["text_color"] = rgbaColorToHexRGB(
    lerpColor(
      textColorToRGBA(config["minimum"]["text_color"]),
      textColorToRGBA(config["maximum"]["text_color"]),
      maxweight
    )
  );
  ret["fill_color"] = rgbaColorToHexRGB(
    lerpColor(
      textColorToRGBA(config["minimum"]["fill_color"]),
      textColorToRGBA(config["maximum"]["fill_color"]),
      maxweight
    )
  );
  ret["stroke_color"] = rgbaColorToHexRGB(
    lerpColor(
      textColorToRGBA(config["minimum"]["stroke_color"]),
      textColorToRGBA(config["maximum"]["stroke_color"]),
      maxweight
    )
  );
  ret["circle_radius"] =
    config["minimum"]["circle_radius"] * minweight +
    config["maximum"]["circle_radius"] * maxweight;
  return ret;
}
...
function getVariableForDatataset(dataset) {
  ...
  if (ckan_server.support_eov == true) {
    dataset["eov"].forEach(function(entry) {
      ...
      if (thumb !== undefined) {
        ret_html +=
          "<img src='" + "/asset/images/thumbnails/" + thumb + "'></img>";
      }
    });
  } else {
  ...
  if (ckan_server.support_eov == true) {
    dataset["eov"].forEach(function(entry) {
      ...
      if (catvar !== undefined) {
        ...
        ret_html +=
          "<li>" + i18nStrings.getTranslation(catvar["label"]) + "</li>";
      }
    });
  }
  ret_html += "</ul><br />";
  ...
  dataset["resources"].forEach(function(entry) {
    if (entry["format"] == "PDF") {
      ...
      ret_html +=
        "<a href='" +
        entry["url"] +
        "' class='asset-link' target='_blank' role='button'>PDF</a> ";
    } else if (entry["format"] == "WMS") {
      // add PDF with link
      ret_html +=
        "<a href='" +
        entry["url"] +
        "' class='asset-link' target='_blank' role='button'>WMS</a> ";
    } else if (entry["format"] == "CSV") {
      // add PDF with link
      ret_html +=
        "<a href='" +
        entry["url"] +
        "' class='asset-link' target='_blank' role='button'>CSV</a> ";
    }
  });
  return ret_html;
}
...
function generateCompleteDetailsPanel(dataset) {
  ...
  if (ckan_server.support_eov == true) {
    ...
    ret_html +=
      ...
      i18nStrings.getUIString("category") +
      "</span>";
    ret_html += getCategoriesForDataset(dataset);
  }
  ret_html +=
    "<span class='details_text'>" +
    i18nStrings.getUIString("dataset_description") +
    "</span>";
  if (ckan_server.support_multilanguage) {
    ret_html +=
      "<p class='details_label'>" +
      i18nStrings.getTranslation(dataset["notes_translated"]) +
      "</p>";
  } else {
    ret_html += "<p class='details_label'>" + dataset["notes"] + "</p>";
  }
  ret_html +=
    '<div class="asset-data-links"><span>' +
    i18nStrings.getUIString("dataset_tools") +
    ":</span>";
  ret_html +=
    '<a target="_blank" href="' +
    ckan_server.getURLForDataset(dataset["id"]) +
    '" class="asset-link" target="_blank" role="button">CKAN</a> ';
  ret_html += getToolForDataset(dataset);
  ret_html += "</div><br />";
  ret_html +=
    "<p class='details_label'>" +
    i18nStrings.getUIString("dataset_provider") +
    "</p><a href='" +
    ckan_server.getURLForOrganization(dataset["organization"]["name"]) +
    "' target='_blank'>";
  ret_html +=
    "<span class=''details_text>" +
...
function generateDetailsPanel(dataset) {
  //, language, dataset_id, title, description, provider, link_url, prov_url)
  let ret_html = "<div id='" + dataset["id"] + "'class='asset_details');'>";
  ret_html +=
    "<span class='details_label'>" +
    i18nStrings.getUIString("dataset_title") +
    "</span>";
  if (ckan_server.support_multilanguage) {
    ret_html +=
      "<p class='details_text'>" +
      i18nStrings.getTranslation(dataset["title_translated"]) +
      "</p>";
  } else {
  ...
  ret_html +=
    ...
    dataset["id"] +
    "_collapse" +
    '" role="button" onclick="showDatasetDetailDescription(\'' +
    dataset["id"] +
    "')\">" +
    i18nStrings.getUIString("details") +
    "</a>";

  // check if geomeetry details available for this dataset
  if (datasetHasSpatial(dataset)) {
    ret_html +=
      '<a href="#" onclick="showInGeometryLayer(\'' +
      dataset["id"] +
      '\');");">' +
      i18nStrings.getUIString("map") +
      "</a> ";
  }
...
function displayTotalSearchDetails(total, paging) {
  ...
  stat_html +=
    ...
    ckan_server.getHomeCatalogURL() +
    "'>" +
    i18nStrings.getUIString("more_data_catalog") +
    " " +
    i18nStrings.getUIString("catalog") +
    "</a>";
...
function addAndDisplaydataset(data) {
  ...
  if (ckan_server.currentFilterQuery == ckan_server.lastFilterChange) {
    ...
    if (ckan_server.lastPagedDataIndex < totaldataset) {
      // continue paging
      displayTotalSearchDetails(
        data["result"]["count"],
        ckan_server.lastPagedDataIndex
      );
      // until the weird jquery jsonp bug is corrected, do it by hand!
      let url_ckan = ckan_server.getURLPaginated(
        ckan_server.lastPagedDataIndex,
        ckan_server.resultPageSize
      );
    ...
    if (ckan_server.use_basic_auth) {
      auth_header = {
        Authorization:
          "Basic " +
          btoa(
            ckan_server.basic_auth_user + ":" + ckan_server.basic_auth_password
          )
      };
    }
    if (ckan_server.usejsonp) {
...
function updateDatasetDetails(datasets) {
  ...
  document.getElementById(
    element["id"] + "_collapse"
  ).innerHTML = generateCompleteDetailsPanel(element);
...
function showDatasetDetailDescription(datasetid) {
  ...
  $("#" + datasetid).on("shown.bs.collapse", function() {
    let topPos =
      $("#dataset_desc").scrollTop() + $("#" + datasetid).position().top;
